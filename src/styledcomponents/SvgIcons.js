import React from 'react';
import SvgIcon from 'material-ui/SvgIcon';

export const AddImageIcon = (props) => (
  <SvgIcon {...props}>
    <path d="M3 4V1h2v3h3v2H5v3H3V6H0V4h3zm3 6V7h3V4h7l1.83 2H21c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H5c-1.1 0-2-.9-2-2V10h3zm7 9c2.76 0 5-2.24 5-5s-2.24-5-5-5-5 2.24-5 5 2.24 5 5 5zm-3.2-5c0 1.77 1.43 3.2 3.2 3.2s3.2-1.43 3.2-3.2-1.43-3.2-3.2-3.2-3.2 1.43-3.2 3.2z" />
  </SvgIcon>
);

export const EditIcon = (props) => (
<SvgIcon {...props}>
  <path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34c-.39-.39-1.02-.39-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z" />
</SvgIcon>
);

export const ActionBook = (props) => (
  <SvgIcon {...props}>
    <path d="M18 2H6c-1.1 0-2 .9-2 2v16c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zM6 4h5v8l-2.5-1.5L6 12V4z"/>
  </SvgIcon>
);

export const AvLibraryBooks = (props) => (
  <SvgIcon {...props}>
    <path d="M4 6H2v14c0 1.1.9 2 2 2h14v-2H4V6zm16-4H8c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-1 9H9V9h10v2zm-4 4H9v-2h6v2zm4-8H9V5h10v2z"/>
  </SvgIcon>
);

export const HomeIcon = (props) => (
  <SvgIcon {...props}>
    <path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z" />
  </SvgIcon>
);

export const ActionBuild = (props) => (
  <SvgIcon {...props}>
    <path d="M22.7 19l-9.1-9.1c.9-2.3.4-5-1.5-6.9-2-2-5-2.4-7.4-1.3L9 6 6 9 1.6 4.7C.4 7.1.9 10.1 2.9 12.1c1.9 1.9 4.6 2.4 6.9 1.5l9.1 9.1c.4.4 1 .4 1.4 0l2.3-2.3c.5-.4.5-1.1.1-1.4z"/>
  </SvgIcon>
);

export const AvNote = (props) => (
  <SvgIcon {...props}>
    <path d="M22 10l-6-6H4c-1.1 0-2 .9-2 2v12.01c0 1.1.9 1.99 2 1.99l16-.01c1.1 0 2-.89 2-1.99v-8zm-7-4.5l5.5 5.5H15V5.5z"/>
  </SvgIcon>
);

export const ContentDrafts = (props) => (
  <SvgIcon {...props}>
    <path d="M21.99 8c0-.72-.37-1.35-.94-1.7L12 1 2.95 6.3C2.38 6.65 2 7.28 2 8v10c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2l-.01-10zM12 13L3.74 7.84 12 3l8.26 4.84L12 13z"/>
  </SvgIcon>
);

export const ContentArchive = (props) => (
  <SvgIcon {...props}>
    <path d="M20.54 5.23l-1.39-1.68C18.88 3.21 18.47 3 18 3H6c-.47 0-.88.21-1.16.55L3.46 5.23C3.17 5.57 3 6.02 3 6.5V19c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V6.5c0-.48-.17-.93-.46-1.27zM12 17.5L6.5 12H10v-2h4v2h3.5L12 17.5zM5.12 5l.81-1h12l.94 1H5.12z"/>
  </SvgIcon>
);


export const NavigationArrowDropDown = (props) => (
  <SvgIcon {...props}>
    <path d="M7 10l5 5 5-5z"/>
  </SvgIcon>
);


export const  ActionAssignment = (props) => (
  <SvgIcon {...props}>
    <path d="M19 3h-4.18C14.4 1.84 13.3 1 12 1c-1.3 0-2.4.84-2.82 2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-7 0c.55 0 1 .45 1 1s-.45 1-1 1-1-.45-1-1 .45-1 1-1zm2 14H7v-2h7v2zm3-4H7v-2h10v2zm0-4H7V7h10v2z"/>
  </SvgIcon>
);

export const FileFileUpload = (props) => (
  <SvgIcon {...props}>
    <path d="M9 16h6v-6h4l-7-7-7 7h4zm-4 2h14v2H5z"/>
  </SvgIcon>
);

export const ActionViewList = (props) => (
  <SvgIcon {...props}>
    <path d="M4 14h4v-4H4v4zm0 5h4v-4H4v4zM4 9h4V5H4v4zm5 5h12v-4H9v4zm0 5h12v-4H9v4zM9 5v4h12V5H9z"/>
  </SvgIcon>
);
export const ImageCollectionsBookmark = (props) => (
  <SvgIcon {...props}>
    <path d="M4 6H2v14c0 1.1.9 2 2 2h14v-2H4V6zm16-4H8c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm0 10l-2.5-1.5L15 12V4h5v8z"/>
  </SvgIcon>
);


export const ActionSpeakerNotes = (props) => (
  <SvgIcon {...props}>
    <path d="M20 2H4c-1.1 0-1.99.9-1.99 2L2 22l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zM8 14H6v-2h2v2zm0-3H6V9h2v2zm0-3H6V6h2v2zm7 6h-5v-2h5v2zm3-3h-8V9h8v2zm0-3h-8V6h8v2z"/>
  </SvgIcon>
);

export const AvMovie = (props) => (
  <SvgIcon {...props}>
    <path d="M18 4l2 4h-3l-2-4h-2l2 4h-3l-2-4H8l2 4H7L5 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V4h-4z"/>
  </SvgIcon>
);


export const ActionTimeline = (props) => (
  <SvgIcon {...props}>
    <path d="M23 8c0 1.1-.9 2-2 2-.18 0-.35-.02-.51-.07l-3.56 3.55c.05.16.07.34.07.52 0 1.1-.9 2-2 2s-2-.9-2-2c0-.18.02-.36.07-.52l-2.55-2.55c-.16.05-.34.07-.52.07s-.36-.02-.52-.07l-4.55 4.56c.05.16.07.33.07.51 0 1.1-.9 2-2 2s-2-.9-2-2 .9-2 2-2c.18 0 .35.02.51.07l4.56-4.55C8.02 9.36 8 9.18 8 9c0-1.1.9-2 2-2s2 .9 2 2c0 .18-.02.36-.07.52l2.55 2.55c.16-.05.34-.07.52-.07s.36.02.52.07l3.55-3.56C19.02 8.35 19 8.18 19 8c0-1.1.9-2 2-2s2 .9 2 2z"/>
  </SvgIcon>
);

export const loveButton =() => (
<svg style={{width:'24px',height:'24px'}} viewBox="0 0 24 24">
    <path fill="#000000" d="M12,21.35L10.55,20.03C5.4,15.36 2,12.27 2,8.5C2,5.41 4.42,3 7.5,3C9.24,3 10.91,3.81 12,5.08C13.09,3.81 14.76,3 16.5,3C19.58,3 22,5.41 22,8.5C22,12.27 18.6,15.36 13.45,20.03L12,21.35Z" />
</svg>
);

export const  SocialSchool = (props) => (
  <SvgIcon {...props}>
    <path d="M5 13.18v4L12 21l7-3.82v-4L12 17l-7-3.82zM12 3L1 9l11 6 9-4.91V17h2V9L12 3z"/>
  </SvgIcon>
);

export const ActionThumbUp = (props) => (
  <SvgIcon {...props}>
    <path d="M1 21h4V9H1v12zm22-11c0-1.1-.9-2-2-2h-6.31l.95-4.57.03-.32c0-.41-.17-.79-.44-1.06L14.17 1 7.59 7.59C7.22 7.95 7 8.45 7 9v10c0 1.1.9 2 2 2h9c.83 0 1.54-.5 1.84-1.22l3.02-7.05c.09-.23.14-.47.14-.73v-1.91l-.01-.01L23 10z"/>
  </SvgIcon>
);

export const MapsRateReview = (props) => (
  <SvgIcon {...props}>
    <path d="M20 2H4c-1.1 0-1.99.9-1.99 2L2 22l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zM6 14v-2.47l6.88-6.88c.2-.2.51-.2.71 0l1.77 1.77c.2.2.2.51 0 .71L8.47 14H6zm12 0h-7.5l2-2H18v2z"/>
  </SvgIcon>
);

export const CommunicationContacts = (props) => (
  <SvgIcon {...props}>
    <path d="M20 0H4v2h16V0zM4 24h16v-2H4v2zM20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm-8 2.75c1.24 0 2.25 1.01 2.25 2.25s-1.01 2.25-2.25 2.25S9.75 10.24 9.75 9 10.76 6.75 12 6.75zM17 17H7v-1.5c0-1.67 3.33-2.5 5-2.5s5 .83 5 2.5V17z"/>
  </SvgIcon>
);


export const CommunicationLocationOn = (props) => (
  <SvgIcon {...props}>
    <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/>
  </SvgIcon>
);

export const CommunicationComment = (props) => (
  <SvgIcon {...props}>
    <path d="M21.99 4c0-1.1-.89-2-1.99-2H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h14l4 4-.01-18zM18 14H6v-2h12v2zm0-3H6V9h12v2zm0-3H6V6h12v2z"/>
  </SvgIcon>
);

export const NavigationClose = (props) => (
  <SvgIcon {...props}>
    <path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/>
  </SvgIcon>
);

export const NavigationArrowBack = (props) => (
  <SvgIcon {...props}>
    <path d="M20 11H7.83l5.59-5.59L12 4l-8 8 8 8 1.41-1.41L7.83 13H20v-2z"/>
  </SvgIcon>
);
export const  NavigationArrowForward = (props) => (
  <SvgIcon {...props}>
    <path d="M12 4l-1.41 1.41L16.17 11H4v2h12.17l-5.58 5.59L12 20l8-8z"/>
  </SvgIcon>
);

export const FileFileDownload = (props) => (
  <SvgIcon {...props}>
    <path d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z"/>
  </SvgIcon>
);

export const NavigationFullscreen = (props) => (
  <SvgIcon {...props}>
    <path d="M7 14H5v5h5v-2H7v-3zm-2-4h2V7h3V5H5v5zm12 7h-3v2h5v-5h-2v3zM14 5v2h3v3h2V5h-5z"/>
  </SvgIcon>
);

export const ActionViewDay = (props) => (
  <SvgIcon {...props}>
    <path d="M2 21h19v-3H2v3zM20 8H3c-.55 0-1 .45-1 1v6c0 .55.45 1 1 1h17c.55 0 1-.45 1-1V9c0-.55-.45-1-1-1zM2 3v3h19V3H2z"/>
  </SvgIcon>
);
export const ActionPageview = (props) => (
  <SvgIcon {...props}>
    <path d="M11.5 9C10.12 9 9 10.12 9 11.5s1.12 2.5 2.5 2.5 2.5-1.12 2.5-2.5S12.88 9 11.5 9zM20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm-3.21 14.21l-2.91-2.91c-.69.44-1.51.7-2.39.7C9.01 16 7 13.99 7 11.5S9.01 7 11.5 7 16 9.01 16 11.5c0 .88-.26 1.69-.7 2.39l2.91 2.9-1.42 1.42z"/>
  </SvgIcon>
);

export const ActionViewArray = (props) => (
  <SvgIcon {...props}>
    <path d="M4 18h3V5H4v13zM18 5v13h3V5h-3zM8 18h9V5H8v13z"/>
  </SvgIcon>
);

export const Room =( props) =>(
  <SvgIcon {...props}>
    <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z" />
  </SvgIcon>
);

export const LocationCity = props =>
    <SvgIcon {...props}>
      <path d="M15 11V5l-3-3-3 3v2H3v14h18V11h-6zm-8 8H5v-2h2v2zm0-4H5v-2h2v2zm0-4H5V9h2v2zm6 8h-2v-2h2v2zm0-4h-2v-2h2v2zm0-4h-2V9h2v2zm0-4h-2V5h2v2zm6 12h-2v-2h2v2zm0-4h-2v-2h2v2z" />
    </SvgIcon>;

export const DeveloperBoard = props =>
      <SvgIcon {...props}>
        <path d="M22 9V7h-2V5c0-1.1-.9-2-2-2H4c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2v-2h2v-2h-2v-2h2v-2h-2V9h2zm-4 10H4V5h14v14zM6 13h5v4H6zm6-6h4v3h-4zM6 7h5v5H6zm6 4h4v6h-4z" />
      </SvgIcon>;
export const NavigationMenu = (props) => (
        <SvgIcon {...props}>
          <path d="M3 18h18v-2H3v2zm0-5h18v-2H3v2zm0-7v2h18V6H3z"/>
        </SvgIcon>
      );
export const NotificationsNone = props =>
        <SvgIcon {...props}>
          <path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.9 2 2 2zm6-6v-5c0-3.07-1.63-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.64 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2zm-2 1H8v-6c0-2.48 1.51-4.5 4-4.5s4 2.02 4 4.5v6z" />
        </SvgIcon>;
export const Edit = props =>
          <SvgIcon {...props}>
            <path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34c-.39-.39-1.02-.39-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z" />
          </SvgIcon>;

export const QuestionAnswer = props =>
                  <SvgIcon {...props}>
                    <path d="M21 6h-2v9H6v2c0 .55.45 1 1 1h11l4 4V7c0-.55-.45-1-1-1zm-4 6V3c0-.55-.45-1-1-1H3c-.55 0-1 .45-1 1v14l4-4h10c.55 0 1-.45 1-1z" />
                  </SvgIcon>;
export const AttachFile = props =>
        <SvgIcon {...props}>
          <path d="M16.5 6v11.5c0 2.21-1.79 4-4 4s-4-1.79-4-4V5c0-1.38 1.12-2.5 2.5-2.5s2.5 1.12 2.5 2.5v10.5c0 .55-.45 1-1 1s-1-.45-1-1V6H10v9.5c0 1.38 1.12 2.5 2.5 2.5s2.5-1.12 2.5-2.5V5c0-2.21-1.79-4-4-4S7 2.79 7 5v12.5c0 3.04 2.46 5.5 5.5 5.5s5.5-2.46 5.5-5.5V6h-1.5z" />
        </SvgIcon>
export const EvaluateIcon = props =>
       <SvgIcon {...props}>
       <path d="M0 0h24v24H0z" fill="none"/>
       <path d="M19 3h-4.18C14.4 1.84 13.3 1 12 1c-1.3 0-2.4.84-2.82 2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-7 0c.55 0 1 .45 1 1s-.45 1-1 1-1-.45-1-1 .45-1 1-1zm-2 14l-4-4 1.41-1.41L10 14.17l6.59-6.59L18 9l-8 8z"/>
       </SvgIcon>

export const ChatOutline= (props) => (
         <SvgIcon {...props}>
           		<path id="XMLID_4_" d="M4,4h16v12H5.17L4,17.17V4L4,4 M4,2C2.9,2,2.01,2.9,2.01,4L2,22l4-4h14c1.1,0,2-0.9,2-2V4c0-1.1-0.9-2-2-2
           			H4L4,2z"/>
           		<polygon id="XMLID_3_" points="6,12 14,12 14,14 6,14 6,12 		"/>
           		<polygon id="XMLID_2_" points="6,9 18,9 18,11 6,11 6,9 		"/>
           		<polygon id="XMLID_1_" points="6,6 18,6 18,8 6,8 6,6 		"/>
         </SvgIcon>
       );

export const DeleteOutline = (props) => (
  <SvgIcon {...props}>
    <g id="Header_x2F_BG" display="none">
    	<rect x="-270" y="-270" display="inline" fill="#F1F1F2" width="520" height="520"/>
    </g>
    <g id="Bounding_Boxes">
    	<g id="ui_x5F_spec_x5F_header_copy_3">
    	</g>
    	<path fill="none" d="M0,0h24v24H0V0z"/>
    </g>
    <g id="Rounded" display="none">
    	<g id="ui_x5F_spec_x5F_header_copy_5" display="inline">
    	</g>
    	<path display="inline" d="M6,19c0,1.1,0.9,2,2,2h8c1.1,0,2-0.9,2-2V9c0-1.1-0.9-2-2-2H8C6.9,7,6,7.9,6,9V19z M18,4h-2.5l-0.71-0.71
    		C14.61,3.11,14.35,3,14.09,3H9.91C9.65,3,9.39,3.11,9.21,3.29L8.5,4H6C5.45,4,5,4.45,5,5v0c0,0.55,0.45,1,1,1h12c0.55,0,1-0.45,1-1
    		v0C19,4.45,18.55,4,18,4z"/>
    </g>
    <g id="Sharp" display="none">
    	<g id="ui_x5F_spec_x5F_header_copy_4" display="inline">
    	</g>
    	<path display="inline" d="M6,21h12V7H6V21z M19,4h-3.5l-1-1h-5l-1,1H5v2h14V4z"/>
    </g>
    <g id="Outline">
    	<g id="ui_x5F_spec_x5F_header">
    	</g>
    	<path id="XMLID_282_" d="M16,9v10H8V9H16 M14.5,3h-5l-1,1H5v2h14V4h-3.5L14.5,3L14.5,3z M18,7H6v12c0,1.1,0.9,2,2,2h8
    		c1.1,0,2-0.9,2-2V7L18,7z"/>
    </g>
    <g id="Duotone" display="none">
    	<g id="ui_x5F_spec_x5F_header_copy_2" display="inline">
    	</g>
    	<g display="inline">
    		<rect x="8" y="9" opacity="0.3" width="8" height="10"/>
    		<g>
    			<polygon points="15.5,4 14.5,3 9.5,3 8.5,4 5,4 5,6 19,6 19,4 			"/>
    			<path d="M6,19c0,1.1,0.9,2,2,2h8c1.1,0,2-0.9,2-2V7H6V19z M8,9h8v10H8V9z"/>
    		</g>
    	</g>
    </g>
    <g id="Fill" display="none">
    	<g id="ui_x5F_spec_x5F_header_copy" display="inline">
    	</g>
    	<path display="inline" d="M6,19c0,1.1,0.9,2,2,2h8c1.1,0,2-0.9,2-2V7H6V19z M19,4h-3.5l-1-1h-5l-1,1H5v2h14V4z"/>
    </g>
    <g id="nyt_x5F_exporter_x5F_info" display="none">
    </g>
  </SvgIcon>
)
export const BugOutline = (props) => (
   <SvgIcon {...props}>
     <g id="Header_x2F_BG" display="none">
      <rect x="-182" y="-182" display="inline" fill="#F1F1F2" width="520" height="520"/>
     </g>
     <g id="Bounding_Boxes">
      <g id="ui_x5F_spec_x5F_header_copy_3">
      </g>
      <path fill="none" d="M0,0h24v24H0V0z"/>
     </g>
     <g id="Rounded" display="none">
      <g id="ui_x5F_spec_x5F_header_copy_5" display="inline">
      </g>
      <path display="inline" d="M19,8h-1.81c-0.45-0.78-1.07-1.45-1.82-1.96l0.93-0.93c0.39-0.39,0.39-1.02,0-1.41v0
        c-0.39-0.39-1.02-0.39-1.41,0l-1.47,1.47C12.96,5.06,12.49,5,12,5s-0.96,0.06-1.41,0.17L9.11,3.7c-0.39-0.39-1.02-0.39-1.41,0l0,0
        c-0.39,0.39-0.39,1.02,0,1.41l0.92,0.93C7.88,6.55,7.26,7.22,6.81,8H5C4.45,8,4,8.45,4,9v0c0,0.55,0.45,1,1,1h1.09
        C6.04,10.33,6,10.66,6,11v1H5c-0.55,0-1,0.45-1,1v0c0,0.55,0.45,1,1,1h1v1c0,0.34,0.04,0.67,0.09,1H5c-0.55,0-1,0.45-1,1v0
        c0,0.55,0.45,1,1,1h1.81c1.04,1.79,2.97,3,5.19,3s4.15-1.21,5.19-3H19c0.55,0,1-0.45,1-1v0c0-0.55-0.45-1-1-1h-1.09
        c0.05-0.33,0.09-0.66,0.09-1v-1h1c0.55,0,1-0.45,1-1v0c0-0.55-0.45-1-1-1h-1v-1c0-0.34-0.04-0.67-0.09-1H19c0.55,0,1-0.45,1-1v0
        C20,8.45,19.55,8,19,8z M13,16h-2c-0.55,0-1-0.45-1-1v0c0-0.55,0.45-1,1-1h2c0.55,0,1,0.45,1,1v0C14,15.55,13.55,16,13,16z M13,12
        h-2c-0.55,0-1-0.45-1-1v0c0-0.55,0.45-1,1-1h2c0.55,0,1,0.45,1,1v0C14,11.55,13.55,12,13,12z"/>
     </g>
     <g id="Sharp" display="none">
      <g id="ui_x5F_spec_x5F_header_copy_4" display="inline">
      </g>
      <path display="inline" d="M20,8h-2.81c-0.45-0.78-1.07-1.45-1.82-1.96L17,4.41L15.59,3l-2.17,2.17C12.96,5.06,12.49,5,12,5
        s-0.96,0.06-1.41,0.17L8.41,3L7,4.41l1.62,1.63C7.88,6.55,7.26,7.22,6.81,8H4v2h2.09C6.04,10.33,6,10.66,6,11v1H4v2h2v1
        c0,0.34,0.04,0.67,0.09,1H4v2h2.81c1.04,1.79,2.97,3,5.19,3s4.15-1.21,5.19-3H20v-2h-2.09c0.05-0.33,0.09-0.66,0.09-1v-1h2v-2h-2
        v-1c0-0.34-0.04-0.67-0.09-1H20V8z M14,16h-4v-2h4V16z M14,12h-4v-2h4V12z"/>
     </g>
     <g id="Outline">
      <g id="ui_x5F_spec_x5F_header">
      </g>
      <g>
        <path d="M20,8h-2.81c-0.45-0.78-1.07-1.45-1.82-1.96L17,4.41L15.59,3l-2.17,2.17C12.96,5.06,12.49,5,12,5s-0.96,0.06-1.41,0.17
          L8.41,3L7,4.41l1.62,1.63C7.88,6.55,7.26,7.22,6.81,8H4v2h2.09C6.04,10.33,6,10.66,6,11v1H4v2h2v1c0,0.34,0.04,0.67,0.09,1H4v2
          h2.81c1.04,1.79,2.97,3,5.19,3c2.22,0,4.15-1.21,5.19-3H20v-2h-2.09c0.05-0.33,0.09-0.66,0.09-1v-1h2v-2h-2v-1
          c0-0.34-0.04-0.67-0.09-1H20V8z M16,12v2v1c0,0.22-0.03,0.47-0.07,0.7l-0.1,0.65L15.46,17c-0.72,1.24-2.04,2-3.46,2
          s-2.74-0.77-3.46-2l-0.37-0.64l-0.1-0.65C8.03,15.48,8,15.23,8,15v-1v-2v-1c0-0.23,0.03-0.48,0.07-0.7l0.1-0.65L8.54,9
          c0.3-0.52,0.72-0.97,1.21-1.31l0.57-0.39l0.74-0.18C11.37,7.04,11.69,7,12,7c0.32,0,0.63,0.04,0.95,0.12l0.68,0.16l0.61,0.42
          c0.5,0.34,0.91,0.78,1.21,1.31l0.38,0.65l0.1,0.65C15.97,10.53,16,10.78,16,11V12z"/>
        <rect x="10" y="14" width="4" height="2"/>
        <rect x="10" y="10" width="4" height="2"/>
      </g>
     </g>
     <g id="Duotone" display="none">
      <g id="ui_x5F_spec_x5F_header_copy_2" display="inline">
      </g>
      <g display="inline">
        <path opacity="0.3" d="M15.83,9.65L15.46,9c-0.3-0.53-0.71-0.96-1.21-1.31l-0.61-0.42l-0.68-0.16C12.63,7.04,12.32,7,12,7
          c-0.31,0-0.63,0.04-0.94,0.11l-0.74,0.18L9.75,7.69C9.26,8.03,8.84,8.48,8.54,9L8.17,9.65l-0.1,0.65C8.03,10.53,8,10.78,8,11v1v2
          v1c0,0.22,0.03,0.47,0.07,0.7l0.1,0.65L8.54,17c0.72,1.24,2.04,2,3.46,2c1.42,0,2.74-0.77,3.46-2l0.37-0.64l0.1-0.65
          C15.97,15.47,16,15.22,16,15v-1v-2v-1c0-0.22-0.03-0.47-0.07-0.7L15.83,9.65z M14,16h-4v-2h4V16z M14,12h-4v-2h4V12z"/>
        <g>
          <path d="M20,8h-2.81c-0.45-0.78-1.07-1.45-1.82-1.96L17,4.41L15.59,3l-2.17,2.17C12.96,5.06,12.49,5,12,5s-0.96,0.06-1.41,0.17
            L8.41,3L7,4.41l1.62,1.63C7.88,6.55,7.26,7.22,6.81,8H4v2h2.09C6.04,10.33,6,10.66,6,11v1H4v2h2v1c0,0.34,0.04,0.67,0.09,1H4v2
            h2.81c1.04,1.79,2.97,3,5.19,3c2.22,0,4.15-1.21,5.19-3H20v-2h-2.09c0.05-0.33,0.09-0.66,0.09-1v-1h2v-2h-2v-1
            c0-0.34-0.04-0.67-0.09-1H20V8z M16,12v2v1c0,0.22-0.03,0.47-0.07,0.7l-0.1,0.65L15.46,17c-0.72,1.24-2.04,2-3.46,2
            c-1.42,0-2.74-0.77-3.46-2l-0.37-0.64l-0.1-0.65C8.03,15.47,8,15.22,8,15v-1v-2v-1c0-0.22,0.03-0.47,0.07-0.7l0.1-0.65L8.54,9
            c0.3-0.52,0.72-0.97,1.21-1.31l0.57-0.39l0.74-0.18C11.37,7.04,11.69,7,12,7c0.32,0,0.63,0.04,0.95,0.12l0.68,0.16l0.61,0.42
            c0.5,0.34,0.91,0.78,1.21,1.31l0.38,0.65l0.1,0.65C15.97,10.53,16,10.78,16,11V12z"/>
          <rect x="10" y="14" width="4" height="2"/>
          <rect x="10" y="10" width="4" height="2"/>
        </g>
      </g>
     </g>
     <g id="Fill" display="none">
      <g id="ui_x5F_spec_x5F_header_copy" display="inline">
      </g>
      <path display="inline" d="M20,8h-2.81c-0.45-0.78-1.07-1.45-1.82-1.96L17,4.41L15.59,3l-2.17,2.17C12.96,5.06,12.49,5,12,5
        s-0.96,0.06-1.41,0.17L8.41,3L7,4.41l1.62,1.63C7.88,6.55,7.26,7.22,6.81,8H4v2h2.09C6.04,10.33,6,10.66,6,11v1H4v2h2v1
        c0,0.34,0.04,0.67,0.09,1H4v2h2.81c1.04,1.79,2.97,3,5.19,3s4.15-1.21,5.19-3H20v-2h-2.09c0.05-0.33,0.09-0.66,0.09-1v-1h2v-2h-2
        v-1c0-0.34-0.04-0.67-0.09-1H20V8z M14,16h-4v-2h4V16z M14,12h-4v-2h4V12z"/>
     </g>
     <g id="nyt_x5F_exporter_x5F_info" display="none">
     </g>
    </SvgIcon>
);
